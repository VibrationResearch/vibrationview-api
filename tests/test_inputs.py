...
class TestInputConfiguration:
    ...
    @pytest.mark.config
    def test_enable_teds_and_input_mode2(self):
        """Test EnableTEDS property and InputMode2 method"""
        try:
            num_channels = self.vv.GetHardwareInputChannels()
            assert num_channels is not None and num_channels > 0
            logger.info(f"Testing EnableTEDS and InputMode2 for {num_channels} channels")
            
            functions_tested = 0
            
            for channel_index in range(min(num_channels, 4)):  # Test first 4 channels
                logger.info(f"Testing EnableTEDS and InputMode2 for channel {channel_index+1}")
                
                try:
                    # Test EnableTEDS getter
                    current_teds_enable = self.vv.EnableTEDS(channel_index)
                    if current_teds_enable is None:
                        logger.warning(f"Channel {channel_index+1} does not support TEDS or EnableTEDS returned None")
                        continue
                    logger.info(f"Channel {channel_index+1} current TEDS enable: {current_teds_enable}")
                    
                    # Test EnableTEDS setter - toggle the value
                    new_teds_enable = not current_teds_enable
                    result = self.vv.EnableTEDS(channel_index, new_teds_enable)
                    assert result == new_teds_enable
                    logger.info(f"Channel {channel_index+1} TEDS enable set to: {new_teds_enable}")
                    
                    # Verify the change took effect
                    verify_teds_enable = self.vv.EnableTEDS(channel_index)
                    assert verify_teds_enable == new_teds_enable
                    logger.info(f"Channel {channel_index+1} TEDS enable verified: {verify_teds_enable}")
                    
                    # Restore original value
                    self.vv.EnableTEDS(channel_index, current_teds_enable)
                    
                    # Test InputMode2 method
                    # Get current channel settings first
                    try:
                        cap_coupled = self.vv.InputCapacitorCoupled(channel_index)
                        power_source = self.vv.InputAccelPowerSource(channel_index)
                        differential = self.vv.InputDifferential(channel_index)
                        
                        logger.info(f"Channel {channel_index+1} current settings: cap={cap_coupled}, "
                                    f"power={power_source}, diff={differential}, teds={current_teds_enable}")
                        
                        # Test InputMode2 with current settings
                        mode_result = self.vv.InputMode2(channel_index, power_source, cap_coupled, 
                                                        differential, current_teds_enable)
                        if mode_result is None:
                            logger.warning(f"Channel {channel_index+1} InputMode2 returned None - may not be supported")
                            # Still count this as tested since we got a response
                            functions_tested += 1
                            continue
                        logger.info(f"Channel {channel_index+1} InputMode2 result: {mode_result}")
                        
                        # Test InputMode2 with TEDS enabled
                        mode_result_teds = self.vv.InputMode2(channel_index, power_source, cap_coupled, 
                                                             differential, True)
                        if mode_result_teds is None:
                            logger.warning(f"Channel {channel_index+1} InputMode2 with TEDS enabled returned None - may not be supported")
                        else:
                            logger.info(f"Channel {channel_index+1} InputMode2 with TEDS enabled result: {mode_result_teds}")
                        
                        # Verify TEDS was enabled
                        final_teds_enable = self.vv.EnableTEDS(channel_index)
                        logger.info(f"Channel {channel_index+1} final TEDS enable after InputMode2: {final_teds_enable}")
                        
                        functions_tested += 1
                        
                    except Exception as mode_error:
                        error_info = ExtractComErrorInfo(mode_error)
                        logger.warning(f"Error testing InputMode2 for channel {channel_index+1}: {error_info}")
                    
                except Exception as e:
                    error_info = ExtractComErrorInfo(e)
                    logger.warning(f"Error testing EnableTEDS for channel {channel_index+1}: {error_info}")
            
            if functions_tested == 0:
                pytest.skip("Could not test EnableTEDS and InputMode2 for any channel")
            else:
                logger.info(f"Successfully tested EnableTEDS and InputMode2 for {functions_tested} channels")
            
        except Exception as e:
            error_info = ExtractComErrorInfo(e)
            logger.error(f"Error in test_enable_teds_and_input_mode2: {error_info}")
            pytest.fail(f"Error in test_enable_teds_and_input_mode2: {error_info}")
